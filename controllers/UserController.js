...
const generateAccessToken = (user) => {
    return jwt.sign({ 
        ...
        is_scientist: user.is_scientist,
...
module.exports = {
    ...
    async index(req, res){
        ...
        try{
            ...
            if(user.is_scientist){
                const cientista = await Scientist.findByPk(id);
                const { register_finished, register, username, birthday, gender, periode, city, state, institute, ...outroDado } = cientista;
            return res.status(200).json(Object.assign({}, other, {register_finished, register, username, birthday, gender, periode, city, state, institute}));
                ...
                const usuario_comum = await User.findByPk(id);
                const { register_finished, register, username, birthday, gender, periode, city, state, institute, groups, ...otherA } = usuario_comum;
        ...
        try{
            if(type){
                const users = await User.findAll({
                    where: { is_scientist: true},
                    attributes: ['id', 'email', 'is_scientist', 'is_admin', 'is_google_login'],
                    include:{
                        association: type
                    },
                })
    ...
    async updateCientista(req, res) {
        ...
        const cientista = await Scientist.findByPk(req.params.id);
        if(cientista){
            await cientista.update(newUser);
            const  accessToken = generateAccessToken({
                id: newUser.id,
                is_scientist: newUserData.is_scientist,
